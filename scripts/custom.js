...
function deleteArea() {
	var area_id = $("div[rel|='selected']").attr('id').split('area_');
...
function generateImageMap() {
	var newWindow = window.open('', 'Preview', 600, 500);
	var html = "<html><head><title>Preview</title><style>img{border:none;}</style></head><body><h1>Preview</h1></body></html>";
	var basePath = getBasePath();
	var image = '<img id="preview_image" src="file:///' + basePath + $("img#photo").attr('src') + '" usemap="#generatedMap">';
	...
	$("[id^=area_]").each(function() {
		...
		var href = "file:///" + basePath + '#' + $(this).attr('title');
		imageMapAreas += '<area shape="rect" title="Spana in ' + href + '" coords="'+x1+','+y1+','+x2+','+y2+'" href="' + href + '" alt="' + href + '">';
	});
	...
	$(newWindow.document.body).append(imageMapAreas);
}
...
$(document).ready(function () {
	...
	$('img#photo').mousedown(function(e){
		...
		var newDiv = "<div id='area_" + currentDiv + "' title='' rel='selected' onclick='selectArea(" + currentDiv + ");' style='position: absolute; top: " + y + "px; left: " + x + "px; width: " + width + "px; height: " + height + "px;'></div>"; 
